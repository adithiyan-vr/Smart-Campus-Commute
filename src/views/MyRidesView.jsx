          ...
          ridesToDisplay.map(ride => {
            ...
            return (
              ...
              >
                ...
                {/* Past Rides: Rate Driver & Carbon Offset */}
                {ride.status === 'COMPLETED' && (
                  <div className="pt-2 border-t border-surface-container flex items-center justify-between gap-2">
                    <span className="text-[11px] text-emerald-700 font-semibold flex items-center gap-1">
                      <span className="material-symbols-outlined text-sm">eco</span>
                      <span>Saved {ride.carbonSavedKg || '2.4'}kg CO₂</span>
                    </span>
                    <button
                      id={`btn-rate-${ride.id}`}
                      onClick={() => onRateRide && onRateRide(ride)}
                      className="px-3 py-1 bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-300 rounded-xl font-headline font-bold text-xs flex items-center gap-1 transition-all shadow-xs active:scale-95"
                    >
                      <span className="material-symbols-outlined text-sm text-amber-600" style={{ fontVariationSettings: "'FILL' 1" }}>star</span>
                      <span>{ride.userRating ? `Rated ★ ${ride.userRating}` : 'Rate Commute'}</span>
                    </button>
                  </div>
                )}
