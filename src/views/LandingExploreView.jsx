...
import { matchesTimeSlot, sortRidesByTimeProximity, CAMPUS_TIME_SLOTS } from '../services/timeMatcher';
...
export default function LandingExploreView({ 
  ...
  }, [waitingMembers, femaleOnly, activeFilterPill, departTime]);
  ...
  const filteredRides = React.useMemo(() => {
    const list = rides.filter(r => {
      ...
      // Strict exact match for departure time
      if (departTime && !matchesTimeSlot(r.departureTime, departTime, 0)) return false;
      return true;
    });

    return departTime ? sortRidesByTimeProximity(list, departTime) : list;
  }, [rides, femaleOnly, activeFilterPill, departTime]);
  ...
  return (
    <div className="flex flex-col w-full pb-8">
      ...
      <section className="px-4 mt-2">
        <div className="w-full bg-surface-container-lowest rounded-xl shadow-[0_8px_24px_-4px_rgba(11,28,48,0.08)] p-4 flex flex-col gap-3 border border-surface-container-high/60">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <div className="flex items-center gap-2.5 bg-surface-container-low px-3 py-2 rounded-xl relative">
              <span className="material-symbols-outlined text-secondary text-lg flex-shrink-0">schedule</span>
              <div className="flex flex-col min-w-0 flex-1">
                <label className="font-label-sm text-secondary uppercase font-semibold text-[10px]">Depart Time Slot</label>
                <input
                  id="input-landing-depart-time"
                  className="bg-transparent border-0 outline-none font-body-md text-on-surface font-medium text-xs truncate w-full"
                  placeholder="e.g. 8:15 AM or All Times"
                  value={departTime}
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none pt-0.5">
            ...
            {CAMPUS_TIME_SLOTS.map(slot => (
              <button
                key={slot.id}
                type="button"
                onClick={() => setDepartTime(slot.value ? `Today, ${slot.value}` : '')}
      ...
      <section className="px-4 mt-5">
        ...
        {/* Active Time Slot Filter Indicator */}
        {departTime && (
          <div className="flex items-center justify-between px-3 py-2 mb-3 rounded-xl bg-primary/10 border border-primary/20 text-xs animate-fadeIn">
            <span className="flex items-center gap-1.5 font-headline font-bold text-primary text-xs">
              <span className="material-symbols-outlined text-base">schedule</span>
              <span>Departure Time Filter: ~{departTime}</span>
            </span>
            <button
              onClick={() => setDepartTime('')}
              className="text-[11px] font-headline font-bold text-secondary hover:text-primary underline flex items-center gap-0.5"
            >
              <span>Show all times</span>
            ...
            {filteredRides.length === 0 ? (
              <div className="bg-surface-container-lowest p-8 rounded-2xl text-center space-y-3 border border-surface-container-high shadow-xs">
                ...
                <h4 className="font-headline font-bold text-sm text-on-surface">
                  No rides found departing around {departTime || 'this time'}
                </h4>
                <p className="text-xs text-secondary max-w-xs mx-auto">
                  No driver routes currently depart in this time window. Try selecting another time slot or clearing the time filter.
                </p>
                {departTime && (
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <span>Show All Available Rides</span>
                  </button>
                )}
              </div>
            ) : (
              filteredRides.map(ride => (
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <span className="block font-label-sm text-emerald-700 font-bold text-[10px]">or 25 🌱 Credits</span>
                  </div>
                </div>
