...
import RideBookingPaymentModal from './components/RideBookingPaymentModal';
...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribeUsers = database.subscribe((updatedUsers) => {
      ...
      if (active) {
        const fresh = updatedUsers.find(u => u.id === active.id || (u.email && u.email.toLowerCase() === (active.email || '').toLowerCase()));
        if (fresh && (fresh.ecoCredits !== active.ecoCredits || fresh.status !== active.status)) {
          const merged = { ...active, ...fresh };
          setCurrentUser(merged);
          database.setActiveSession(merged);
        }
      }
    });
  ...
  // Eco-Credits Ride Booking & Payment Modal State
  const [selectedBookingRide, setSelectedBookingRide] = useState(null);
  const [isBookingPaymentModalOpen, setIsBookingPaymentModalOpen] = useState(false);

  // Post-Ride Rating Modal State (Screen 8)
  ...
  // Booking Flow from Landing / Find Ride - opens payment selector modal
  const handleBookSeat = (ride) => {
    ...
    setSelectedBookingRide(ride);
    setIsBookingPaymentModalOpen(true);
  };

  // Execution of booking upon student payment method confirmation
  const handleConfirmBookingPayment = async ({ ride, paymentMethod, creditsPerSeat = 25 }) => {
    try {
      const result = await database.bookSeat(ride.id, {
      ...
      if (result.user && currentUser && (result.user.id === currentUser.id || result.user.email === currentUser.email)) {
    ...
    const newBooking = {
      ...
      price: paymentMethod === 'eco_credits' ? '25 🌱 Credits' : `₹${Math.round(ride.pricePerSeat)}`,
      status: 'CONFIRMED',
      seatsBooked: 1,
    ...
    if (paymentMethod === 'eco_credits') {
      showToast(`Reserved 1 seat with ${ride.driver.name}! Paid 25 Eco-Credits 🌱`, 'Booked with Credits');
    } else {
  ...
  // Driver accepts a waiting member to pick up (Awards +30 Eco-Credits)
  const handlePickUpMember = async (member) => {
    try {
      ...
      if (currentUser) {
        setCurrentUser(prev => ({ ...prev, ecoCredits: (prev.ecoCredits || 120) + 30 }));
      }
    } catch {
    ...
    showToast(`Picked up ${member.passenger.name}! Earned +30 Eco-Credits 🌱`, 'Rider Confirmed');
    setCurrentView('my-rides');
  };

  // Handle ride status updates (Start Ride, Complete Ride -> Awards +25 Eco-Credits & Opens Rating)
  const handleUpdateRideStatus = async (rideId, newStatus) => {
    ...
    if (newStatus === 'COMPLETED') {
      const target = userRides.find(r => r.id === rideId);
      if (target && target.role === 'passenger') {
        setTimeout(() => {
          setRatingModalRide(target);
      ...
      if (currentUser) {
        try {
          const updated = await database.rewardUserCredits(currentUser.id, 25, 'Ride Completed Incentive');
          if (updated) {
            setCurrentUser(prev => ({ ...prev, ...updated }));
            showToast('🌱 Earned +25 Eco-Credits for completing student trip!', 'Eco-Reward');
          }
        } catch {}
      }
    }
  ...
  const handleOpenRatingModal = (ride) => {
    setRatingModalRide(ride);
    setIsRatingModalOpen(true);
  };

  const handleSubmitRating = ({ rideId, rating, tags, note }) => {
    setUserRides(prev => prev.map(r => r.id === rideId ? { ...r, userRating: rating, reviewTags: tags, reviewNote: note } : r));
    showToast(`⭐ Review submitted! Rated ${rating} stars. Thank you for supporting campus sustainability.`, 'Review Recorded');
  };
  ...
  return (
    <div className="bg-surface text-on-surface font-sans min-h-screen flex flex-col">
          ...
          <>
            ...
            {currentView === 'my-rides' && (
              <MyRidesView
                ...
                onRateRide={handleOpenRatingModal}
              />
            )}
      ...
      {/* Ride Booking & Eco-Credits Payment Modal */}
      <RideBookingPaymentModal
        isOpen={isBookingPaymentModalOpen}
        onClose={() => {
          setIsBookingPaymentModalOpen(false);
          setSelectedBookingRide(null);
        }}
        ride={selectedBookingRide}
        user={currentUser}
        onConfirmBooking={handleConfirmBookingPayment}
      />

      {/* Post-Ride Rating Modal (Screen 8) */}
      <PostRideRatingModal
