            ...
            if (fresh.status === 'Suspended') {
              ...
              if (typeof window !== 'undefined') {
                window.dispatchEvent(new CustomEvent('campuscommute:session_expired', {
                  detail: { user: active, reason: 'Suspended' }
                }));
              }
            } else {
...
export const database = {
  ...
  registerUser: async ({ name, email, password, role = 'rider', major, studentId, vehicle = null }) => {
    ...
    try {
      ...
      if (res.ok && data.success && data.user) {
        ...
        try { await syncFromCentralServer(); } catch {}
        return data.user;
      } else {
        throw new Error(data.error || 'Registration failed.');
      }
    } catch (err) {
      // If error message is from server (e.g. 403 or specific validation), propagate it
      if (err.message && !err.message.includes('fetch') && !err.message.includes('NetworkError')) {
        throw err;
      }
      // Offline fallback: register in inMemoryUsers
      ...
      let existing = inMemoryUsers.find(u => (u.email || '').toLowerCase() === cleanEmail);
      if (existing) {
        existing.password = cleanPassword;
        existing.name = formattedName;
        existing.role = role;
        database.setActiveSession(existing);
        database._notify();
        return existing;
      }
      const localUser = {
      ...
      database.setActiveSession(localUser);
      database._notify();
      return localUser;
    }
  },
  ...
  loginUser: async ({ email, password, strict = false }) => {
    let cleanEmail = String(email || '').trim().toLowerCase();
    const cleanPassword = String(password || '').trim();
    ...
    try {
      const res = await fetch(getApiUrl('/api/users/login'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: cleanEmail, password: cleanPassword, strict })
      });
      ...
      // If error message is from server (e.g. 401 Incorrect password), propagate it
      if (err.message && !err.message.includes('fetch') && !err.message.includes('NetworkError')) {
        throw err;
      }
      // Offline fallback: verify against inMemoryUsers
      const isAdminAttempt = cleanEmail === 'admin' || cleanEmail === 'admin@university.edu';
      if (isAdminAttempt) {
        const passOk = cleanPassword === 'AdminPass2024!' || cleanPassword === 'admin' || cleanPassword === 'admin123';
        if (passOk) {
          const adminUser = inMemoryUsers.find(u => u.role === 'admin') || DEFAULT_SEED_ADMIN;
          database.setActiveSession(adminUser);
          return adminUser;
        }
        throw new Error('Incorrect administrator password.');
      }
      const localUser = inMemoryUsers.find(u => {
        const uEmail = String(u.email || '').trim().toLowerCase();
        const prefix = uEmail.split('@')[0].toLowerCase();
        return uEmail === cleanEmail || prefix === cleanEmail || (!cleanEmail.includes('@') && uEmail === `${cleanEmail}@university.edu`);
      });
      if (!localUser) {
        if (strict) {
          throw new Error('Account not found. Please register first or verify your email.');
        }
        ...
        inMemoryUsers.push(newLocal);
        database.setActiveSession(newLocal);
        database._notify();
        return newLocal;
      }
      if (String(localUser.password || '').trim() !== cleanPassword) {
        throw new Error('Incorrect password. Please verify and try again.');
      }
      if (localUser.status === 'Suspended') {
        throw new Error('This account has been suspended by campus administrators.');
      }
      database.setActiveSession(localUser);
      return localUser;
    }
  ...
  adminLogin: async ({ email, password }) => {
    ...
    try {
      ...
      const isAdmin = cleanEmail === 'admin' || cleanEmail === 'admin@university.edu';
      const passOk = cleanPassword === 'AdminPass2024!' || cleanPassword === 'admin' || cleanPassword === 'admin123';
      if (isAdmin && passOk) {
        const adminUser = inMemoryUsers.find(u => u.role === 'admin') || DEFAULT_SEED_ADMIN;
        database.setActiveSession(adminUser);
  ...
  bookSeat: async (rideId, bookingOptions = {}) => {
    const active = database.getActiveSession();
    const payload = {
      passengerId: bookingOptions.passengerId || active?.id,
      passengerEmail: bookingOptions.passengerEmail || active?.email,
      paymentMethod: bookingOptions.paymentMethod || 'cash',
      creditsPerSeat: bookingOptions.creditsPerSeat || 25
    };
    const res = await fetch(getApiUrl(`/api/rides/${rideId}/book`), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (res.ok && data.ride) {
      if (data.rides) inMemoryRides = data.rides;
      if (data.user && active && (data.user.id === active.id || data.user.email === active.email)) {
        database.setActiveSession({ ...active, ...data.user });
      }
      database._notifyRides();
      database._notify();
      return data;
  ...
  acceptWaitingMember: async (memberId, driverInfo = {}) => {
    const active = database.getActiveSession();
    const payload = {
      driverId: driverInfo.driverId || active?.id,
      driverEmail: driverInfo.driverEmail || active?.email
    };
    const res = await fetch(getApiUrl(`/api/rides/waiting/${memberId}/accept`), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload)
    });
    ...
    if (res.ok && data.waitingMembers) {
      inMemoryWaitingMembers = data.waitingMembers;
      if (data.driver && active && (data.driver.id === active.id || data.driver.email === active.email)) {
        database.setActiveSession({ ...active, ...data.driver });
      }
      database._notifyRides();
      database._notify();
      return data.waitingMembers;
    }
    throw new Error(data.error || 'Failed to accept waiting member.');
  },

  rewardUserCredits: async (userId, amount, reason = 'Campus Carpool Reward') => {
    const res = await fetch(getApiUrl(`/api/users/${userId}/credits`), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (res.ok && data.user) {
      const active = database.getActiveSession();
      if (active && (data.user.id === active.id || data.user.email === active.email)) {
        database.setActiveSession({ ...active, ...data.user });
      }
      database._notify();
      return data.user;
