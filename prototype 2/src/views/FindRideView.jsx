...
export default function FindRideView({ 
  ...
  // Filter rides with strict gender guard and route matching (for passengers looking for drivers)
  const searchResults = rides.filter(r => {
  ...
  useEffect(() => {
    if (searchResults.length > 0 && (!selectedRide || !searchResults.some(r => r.id === selectedRide.id))) {
      setSelectedRide(searchResults[0]);
    }
  }, [searchResults]);

  // Keep selected member in sync with filtered waiting members
  useEffect(() => {
    if (filteredWaitingMembers.length > 0 && (!selectedMember || !filteredWaitingMembers.some(m => m.id === selectedMember.id))) {
      setSelectedMember(filteredWaitingMembers[0]);
    }
    ...
    <div className="px-4 py-3 flex flex-col gap-4 pb-20">
      ...
      <div className="grid grid-cols-2 p-1 bg-surface-container rounded-2xl border border-surface-container-high shadow-xs">
        ...
        >
          ...
          <span>Find Drivers ({searchResults.length})</span>
        </button>
          ...
          <>
            <div className="flex justify-between items-center px-1">
              <span className="font-headline font-bold text-xs uppercase tracking-wider text-emerald-800 flex items-center gap-1">
                ...
                <span>Members Waiting to Pick Up ({filteredWaitingMembers.length})</span>
              </span>
          ...
          <>
            <div className="flex justify-between items-center px-1">
              <span className="font-headline font-bold text-xs uppercase tracking-wider text-secondary">
                Matching Driver Routes ({searchResults.length})
              </span>
            ...
            {searchResults.length === 0 ? (
              <div className="bg-surface-container-lowest p-8 rounded-2xl text-center space-y-3 border border-surface-container-high shadow-xs">
                ...
                <p className="text-xs text-secondary max-w-xs mx-auto">
                  No driver routes currently match {source ? `"${source}"` : 'your filters'}. Try resetting your search filters or choosing another corridor.
                </p>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        ...
                        <span className="block text-[10px] text-secondary">per seat</span>
                      </div>
                    </div>
                    ...
                    <div className="mt-3 bg-surface-container-low p-2.5 rounded-xl flex items-center justify-between text-xs">
                      ...
                      <div className="text-right flex-shrink-0">
                        <span className="font-bold text-on-surface block">{ride.departureTime}</span>
                        <span className="text-[11px] text-secondary font-medium block">
